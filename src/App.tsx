...
export default function App() {
  ...
  const handleBuildProposal = (thesis: Thesis) => { setProposalInitialThesis(thesis); setActiveTab("proposal"); };
  ...
  const handleExploreRabbit = (thesis: Thesis) => { setRabbitSeedThesis(thesis); setActiveTab("rabbit"); };
  ...
  return <div className="min-h-screen bg-slate-100/60 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white transition-colors">
    ...
    <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-8">
      ...
      {activeTab === "admin" && <AdminPage currentUser={currentUser} onShowToast={addToast} setActiveTab={setActiveTab} />}
      ...
      {!['landing','research-explorer','search','rabbit','rare','proposal','compare','library','chat','profile','auth','about','contact','ai-tools','admin','api-docs','srs','legal'].includes(activeTab) && <NotFoundPage setActiveTab={setActiveTab} />}
    </main>
